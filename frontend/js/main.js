...
export async function updateNavbar() {
    ...
    if (navIcons) {
        ...
        if (user) {
            ...
            authBtn.innerHTML = `
                <a href="/frontend/pages/User-profile.html" title="View Profile" style="text-decoration: none; color: inherit; display: flex; align-items: center; gap: 5px;">
            ...
            if (!document.querySelector('a[href="/frontend/pages/My-orders.html"]')) {
                const navLinks = document.querySelector('.nav-links');
                if (navLinks) {
                    const ordersLink = document.createElement('li');
                    ordersLink.innerHTML = `<a href="/frontend/pages/My-orders.html">My Orders</a>`;
                    navLinks.appendChild(ordersLink);
                }
            }

            // Add "Farmer Dashboard" link ONLY if the logged-in user is a farmer
            if (user.role === 'farmer' && !document.querySelector('a[href="/frontend/Farmer-Page/Farmer-dashboard.html"]')) {
                const farmerLink = document.createElement('li');
                farmerLink.innerHTML = `<a href="/frontend/Farmer-Page/Farmer-dashboard.html">Farmer Dashboard</a>`;
                ...
                document.querySelectorAll('a[href="/frontend/pages/Become-a-seller.html"]').forEach(link => {
            ...
            authBtn.innerHTML = '<a href="/frontend/pages/login.html" title="Login"><i class="fa-solid fa-user"></i></a>';
        }
    ...
    searchInputs.forEach(input => {
        input.addEventListener('keypress', (e) => {
            if (e.key === 'Enter') {
                ...
                if (query) {
                    window.location.href = `/frontend/pages/Shop.html?search=${encodeURIComponent(query)}`;
                }
            }
        });
    });
